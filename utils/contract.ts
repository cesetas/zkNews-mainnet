...
const getContract = async () => {
  const PTE_KEY = process.env.NEXT_PUBLIC_TEST_PRIVATE_KEY;
  const URL = process.env.NEXT_PUBLIC_TEST_URL;
  const contractAddress = process.env.NEXT_PUBLIC_TEST_CONTRACT_ADDRESS;
